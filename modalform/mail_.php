...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (!empty($_POST['uname']) && || !empty($_POST['uphone']))){
    ...
    if (isset($_POST['formInfo'])) {
	   ...
	   if (isset($_POST['client_summ'])) {
        if (!empty($_POST['client_summ'])){
          $formSumm = strip_tags($_POST['client_summ']);
          $formSummInfoFieldset = "<b>Сумма:</b>";
        }
    }
	  
	     if (isset($_POST['client_monthes'])) {
        if (!empty($_POST['client_monthes'])){
          $formMonthInfo = strip_tags($_POST['client_monthes']);
          $formMonthInfoFieldset = "<b>Срок рассрочки</b>";
        }
    }
    ...
    $message = "$unameFieldset $uname
				...
				$formSummInfoFieldset $formSumm
				$formMonthInfoFieldset $formMonthInfo
				";
